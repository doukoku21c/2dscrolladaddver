...
public class PlayerMove : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (gameManager.totalPoint >= 98000)  // 스테이지9까지의 토탈 스코어가 9.8 만이상이면 씬 endgame1으로 이동
            {
                Time.timeScale = 1;
            ...
            else if (gameManager.totalPoint >= 85000)  // 스테이지9까지의 토탈 스코어가 8.5만 이상이면 씬 endgame2으로 이동
            {
                Time.timeScale = 1;
            ...
            else  // 스코어가 10만 이하이면  씬 endgame3으로 이동 
            {
                Time.timeScale = 1;
